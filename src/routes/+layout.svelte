...
>
  {#key data.url}
    <div in:blur={{duration: 300, delay: 150}} out:blur={{duration: 150}} class="page-content">
      <slot />
    </div>
  {/key}
</div>
...
<style>
  ...
  .layout-container::before {
    ...
    background-image:
      radial-gradient(var(--dot-color, #d66060) var(--dot-size, 0.45px), transparent var(--dot-size, 0.45px)),
      radial-gradient(var(--dot-color, #d66060) var(--dot-size, 0.45px), #a53535 var(--dot-size, 0.45px));
