...
{#if visible}
  <div class="modal-background" transition:fade={{duration: 100}}>
    <div class="modal" transition:scale={{duration: 200, start: 0.95, opacity: 1}}>
      ...
      <p>
        Mods are created and maintained by third-party developers. While the index is curated, we cannot guarantee their
        safety. Please install only if you trust the source.
      </p>
      {#if errorMessage}
        <p class="error-message">{errorMessage}</p>
      {/if}
      <div class="buttons">
        <button class="cancel-button" on:click={handleQuit} disabled={isLoading}> Quit </button>
        <button class="confirm-button" on:click={handleAcknowledge} disabled={isLoading}>
          {#if isLoading}
            Saving...
          {:else}
            I Understand
