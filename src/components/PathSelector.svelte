<script lang="ts">
  ...
  const handlePathSelect = async () => {
    ...
    const selected = await open({
      ...
      multiple: false,
      title: selectMode === "directory" ? "Select Balatro Directory" : "Select Balatro Executable",
      filters: selectMode === "executable" ? [{name: "Executable", extensions: ["exe"]}] : undefined,
    });
    ...
    if (selected) {
      ...
      if (selectMode === "executable") {
        ...
        const lastSlashIndex =
          selectedPath.lastIndexOf("/") !== -1 ? selectedPath.lastIndexOf("/") : selectedPath.lastIndexOf("\\");
      ...
      if (isValid) {
        ...
        addMessage("Balatro path set successfully! You can now manage mods for this installation.", "success");
      } else {
        addMessage("Invalid Balatro path. Please select a directory containing the game files.", "error");
        selectedPath = "";
      }
    }
  };
...
<div class="path-selector">
  ...
  <p class="description-small">
    {#if selectMode === "executable"}
      Select a Balatro executable to manage mods for a specific installation. This is useful for separate vanilla/modded
      setups.
    {:else}
