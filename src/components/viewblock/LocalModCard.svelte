<script lang="ts">
  import {Trash2, ArrowDownToLine, CornerDownRight, Folder} from "lucide-svelte";
  ...
  async function toggleModEnabled(e: Event) {
    ...
    try {
      ...
      console.error(`Failed to toggle mod ${mod.name} enabled state:`, error);
    }
  }
  ...
  async function installOfficialVersion(e: Event) {
    ...
    try {
      ...
      const fullCatalogMod = $modsStore.find(m => m.title.toLowerCase() === mod.catalog_match.title.toLowerCase());

      // Use the dependency info from the full catalog mod if found
      const dependencies = [];
      if (fullCatalogMod) {
        if (fullCatalogMod.requires_steamodded) dependencies.push("Steamodded");
        if (fullCatalogMod.requires_talisman) dependencies.push("Talisman");
      }
      ...
      try {
        ...
        const installedPath: string = await invoke("install_mod", {
          ...
          folderName: fullCatalogMod?.folder_name || catalogMod.title.replace(/\s+/g, ""),
        });
        ...
        if (!pathExists) {
          throw new Error(`Failed to verify installed path: ${installedPath}`);
        }
        ...
        const normalizedLocal = localModPath.toLowerCase().replace(/\\/g, "/");
        const normalizedInstalled = installedPath.toLowerCase().replace(/\\/g, "/");
        ...
        addMessage(`Installed official version of ${mod.catalog_match.title}`, "success");
...
<div class="mod-card" style="--bg-color: {bgColor}; --bg-color-2: {bgColor2};">
  ...
  <div class="button-container">
    {#if mod.catalog_match}
      <button class="folder-button" title="Open mod directory" on:click={openModDirectory}>
        <Folder size={18} />
      </button>
      ...
      <button
        ...
        title={($modEnabledStore[mod.name] ?? isEnabled) ? "Disable Mod" : "Enable Mod"}
      ...
      <button class="trash-button" title="Remove Mod" on:click={uninstallMod}>
        <Trash2 size={18} />
      </button>
    {:else}
      <button class="folder-button" title="Open mod directory" on:click={openModDirectory}>
      ...
      <button class="delete-button" title="Remove Mod" on:click={uninstallMod}>
        <Trash2 size={18} />
        Remove
      </button>
    {/if}
