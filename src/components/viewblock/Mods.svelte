<script lang="ts">
  ...
  async function handleModToggled(): Promise<void> {
    if ($currentCategory === "Installed Mods") {
      ...
      for (const mod of localMods) {
        try {
          const isEnabled = await invoke<boolean>("is_mod_enabled_by_path", {
            modPath: mod.path,
          });
          modEnabledStore.update(s => ({
            ...s,
      ...
      try {
        ...
        for (const mod of localMods) {
          try {
            ...
            console.error(`Failed to check if local mod ${mod.name} is enabled:`, error);
            modEnabledStore.update(s => ({
              ...s,
  ...
  $: hasUpdatesAvailable = Object.values($updateAvailableStore).some(value => value === true);
  ...
  async function updateAllMods(e?: Event) {
    ...
    try {
        ...
        modsToUpdate.map(async mod => {
          try {
            if (mod.title.toLowerCase() === "steamodded") {
              const latestReleaseURL = await invoke<string>("get_latest_steamodded_release");
              await installModFromURL(mod, latestReleaseURL);
            } else if (mod.download_url) {
              const folderName = mod.folder_name || mod.title.replace(/\s+/g, "");
      ...
      if (successful.length > 0) {
        addMessage(`Successfully updated ${successful.length} mod(s).`, "success");
      }
    } catch (error) {
  ...
  async function installModFromURL(mod: Mod, url: string, folder_name: string = "") {
    try {
      ...
      const folderName = folder_name || mod.folder_name || mod.title.replace(/\s+/g, "");
      ...
      for (const mod of $modsStore) {
        ...
        if (isInstalled && installedMod) {
          try {
            ...
            console.error(`Failed to check updates for ${mod.title}:`, error);
          }
        }
      }
...
<div class="container default-scrollbar">
    ...
    {:else}
      <div class="mods-wrapper">
        <div class="controls-container">
          ...
          <div class="pagination-controls" in:fly={{duration: 400, y: 10, opacity: 0.2}}>
            ...
            {:else if localMods.length > 0}
              ...
              {#if enabledLocalMods.length > 0}
                <div class="subsection-header enabled" class:top-margin={localMods.length === 0}>
                  ...
                  <p>
                    {enabledLocalMods.length} mod{enabledLocalMods.length !== 1 ? "s" : ""} active
                  </p>
                </div>
              ...
              {#if disabledLocalMods.length > 0}
                <div class="subsection-header disabled" class:top-margin={localMods.length === 0}>
                  ...
                  <p>
                    {disabledLocalMods.length} mod{disabledLocalMods.length !== 1 ? "s" : ""} inactive
                  </p>
                </div>
                <div class="mods-grid local-mods-grid">
  ...
  {#if $currentModView}
    <ModView mod={$currentModView!} onCheckDependencies={handleDependencyCheck} />
  {/if}
</div>
...
<style>
  ...
  .mods-scroll-container:not(:has(.local-mods-grid)) .subsection-header:first-of-type {
