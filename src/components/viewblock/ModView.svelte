<script lang="ts">
  ...
  import {Download, Trash2, User, ArrowLeft, Github, X, RefreshCw} from "lucide-svelte";
  ...
  async function loadTalismanVersions() {
    if (loadingVersions) return;
    try {
      const cached = await invoke<[string[], number]>("load_versions_cache", {modType: "talisman"});
      if (cached) {
        const [cachedVers, cachedTs] = cached;
        if (Date.now() - cachedTs * 1000 < VERSION_CACHE_DURATION) {
    ...
    const performDownload = async () => {
      try {
        ...
        if (mod.title.toLowerCase() === "steamodded") {
          ...
          if (!pathExists) throw new Error("Installation failed - files not found at destination");
  ...
  function handleMarkdownClick(event: MouseEvent | KeyboardEvent) {
    ...
    if (internalModName) {
      ...
      open(anchor.href).catch(e => console.error("Failed to open link:", e));
    }
  }
        ...
        <div class="button-container">
          ...
          {#if $installationStatus[mod.title]}
            <button
              ...
              title={($modEnabledStore[mod.title] ?? isEnabled) ? "Disable Mod" : "Enable Mod"}
          ...
          {:else}
            ...
            <button
              ...
              disabled={$installationStatus[mod.title] || $loadingStates[mod.title]}
          ...
          {#if $installationStatus[mod.title]}
            <button class="delete-button" title="Remove Mod" onclick={() => uninstallMod(mod)}>
              <Trash2 size={18} />
            </button>
          {/if}
        </div>
              ...
              {#each mod.categories as category (category)}
                <button
                  ...
                  onclick={() => {
                    ...
                    setTimeout(() => {
                      const modsContainer = document.querySelector(".mods-scroll-container");
                      if (modsContainer) {
